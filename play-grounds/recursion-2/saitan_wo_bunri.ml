...
(*koshin: eki_t -> eki_t list -> eki_t list*)
let koshin p v = 
  (*Objective: check if p and q are connected*)
  (*is_connected_and_dist : eki_t -> eki_t -> ekikan_t list -> bool * float *)
  let rec is_connected_and_dist p q lst = match lst with
    [] -> false, 0.
  | first :: rest -> 
      if (first.kiten = p.namae && first.shuten = q.namae) || (first.kiten = q.namae && first.shuten = q.namae) then true, first.kyori
      else is_connected_and_dist p q rest in
        
      List.map (
        (*Objective: check if station p and station q are connected with each other*)
        (* koshin1: eki_t -> eki_t -> eki_t*)
        (fun p q ->
          let (is_connected_pq, p_q_dist) = is_connected_and_dist p q global_ekikan_list in
            if is_connected_pq then
              match (p, q) with
                ({namae = n1; saitan_kyori = s1; temae_list = l1}, {namae = n2; saitan_kyori = s2; temae_list = l2}) ->
                  if s1 +. p_q_dist < s2 then {namae = n2; saitan_kyori = s1 +. p_q_dist; temae_list = n2 :: l1}
                  else q
            else q)
        p) v
...
let saitan_wo_bunri lst =
  ...
  (*Objective: given not determined point set V, extract minimum distance point*)
  (*extract_min : eki_t list -> eki_t*)
  let rec extract_min lst = match lst with
  [] -> {namae = ""; saitan_kyori = infinity; temae_list = []}
  | first :: rest ->
    let min_rest = extract_min rest in
    match (first, min_rest) with
      {namae = n1; saitan_kyori = s1; temae_list = t1}, {namae = n2; saitan_kyori = s2; temae_list = t2} ->
        if s1 < s2 then first
        else min_rest in

  let min_station = extract_min lst in
    match min_station with
      {namae = n1; saitan_kyori = s1; temae_list = t1} ->
        let min_other_list = (List.filter (
          fun eki ->
            match eki with
              {namae = n2; saitan_kyori = s2; temae_list = t2} ->
                n2 = n1
        ) lst) in
                (min_station, min_other_list)
...
let lst_example = make_initial_eki_list seiretsu_list "池袋"
let test = saitan_wo_bunri lst_example = saitan_wo_bunri2 lst_example