...
(* Objective: make eki_t data list from ekimei_t data list
...
(*Objective: initialize eki_t list*)
    ...
    fun eki ->
      match eki with
        {namae = n; saitan_kyori = s; temae_list = t} ->
          if n = kiten then {namae = kiten; saitan_kyori = 0.0; temae_list = [kiten]}
          else eki
  ) lst *)
...
(*Objective: Given start and end, calc shortest path*)
