...
(*Objective: get distance between given two stations*)
(*get_ekikan_kyori: string -> string -> ekimei_t list -> float*)
let rec get_ekikan_kyori st1 st2 lst = match lst with
    [] -> raise Not_found
  | {kiten = kiten; shuten = shuten ; keiyu = keiyu ; kyori = kyori ; jikan = jikan} :: rest ->
      if (st1 = kiten && st2 = shuten) || (st1 = shuten && st2 = kiten) then kyori
      else get_ekikan_kyori st1 st2 rest


(*Objective: tell distance between station A & B if they are connected with*)
(*kyori_wo_hyoji: string -> string -> ekimei_t list -> ekikan_t list -> string*)
let rec kyori_wo_hyoji ekimei1 ekimei2 ekimei_list ekikan_list =
  let st1 = romaji_to_kanji ekimei1 ekimei_list in
    if st1 = "" then ekimei1 ^ " does not exist. "
    else 
      let st2 = romaji_to_kanji ekimei2 ekimei_list in
        if st2 = "" then ekimei2 ^ " does not exist. "
        else 
          let distance = get_ekikan_kyori st1 st2 ekikan_list in
            if distance = infinity then st1 ^ " and " ^ st2 ^ " are not connected directly."
            else "The distance between " ^ st1 ^ " and " ^ st2 ^ " is " ^ string_of_float distance
...
(*Objective: make and initialize eki_t list*)
...
(*Objective: insert ekimei_t data in list*)
(*insert: ekimei_t list -> ekimei_t -> ekimei_t list*)
let rec insert lst data = match lst with
  [] -> [data]
| first :: rest ->
    if first.kana < data.kana then first :: insert rest data
    else data :: first :: rest

(*Objective: sort ekimei_t list by kana-order and remove duplication*)
(*insert_sort: ekimei_t list -> ekimei_t list*)
let rec insert_sort lst = match lst with
  [] -> []
| first :: rest -> insert (insert_sort rest) first

let sorted_list = insert_sort global_ekimei_list
(*Objective: sort and remove duplication*)
(*remove_dupl: ekimei_t list -> ekimei_t list*)
let rec remove_dupl lst = match lst with
    [] -> []
  | first :: [] -> [first]
  | first :: second :: rest -> 
      if first.kana = second.kana then remove_dupl (first :: rest)
      else first :: remove_dupl (second :: rest)

let seiretsu lst = remove_dupl (insert_sort lst)

let seiretsu_list = seiretsu global_ekimei_list
...
(*Objective: Extract shortest path point p from set V*)
...
let saitan_wo_bunri lst =
  ...
  let min_station = List.fold_right (
    fun eki1 eki2 -> if eki1.saitan_kyori < eki2.saitan_kyori then eki1
                     else eki2
  ) lst init_eki in
  let min_other_staion = List.filter (fun eki -> eki.namae != min_station.namae) lst in
  min_station, min_other_staion
...
let rec dijkstra_main eki_list ekikan_list =
  if eki_list = [] then []
  else let min_station, min_other_station = saitan_wo_bunri eki_list in
  let updated_v = koshin min_station min_other_station ekikan_list in
  min_station :: (dijkstra_main updated_v ekikan_list)
...
let dijkstra start_romaji terminal_romaji =
  ...
  let start = romaji_to_kanji start_romaji seiretsu_list in
  let terminal = romaji_to_kanji terminal_romaji seiretsu_list in
  let eki_list = make_initial_eki_list seiretsu_list start in
  let determined_eki_list = dijkstra_main eki_list global_ekikan_list in
  let rec find_terminal eki_namae eki_list =
    match eki_list with
      [] -> {namae = ""; saitan_kyori = infinity; temae_list = []}
    | first :: rest -> if first.namae = eki_namae then first
                       else find_terminal eki_namae rest in
  find_terminal terminal determined_eki_list
...
let insert_ekikan ekikan tree = match ekikan with
    {kiten = kiten; shuten = shuten; keiyu = keiyu; kyori = kyori; jikan = jikan} ->
      let rec support tree ekimei_1 ekimei_2 = match tree with
        Empty -> Node (Empty, ekimei_1, [(ekimei_2, kyori)], Empty)
      | Node (t1, ekimei, lst, t2) -> 
          if ekimei = ekimei_1 
            then Node (t1, ekimei_1, (ekimei_2, kyori) :: lst, t2)
          else if ekimei_1 < ekimei 
            then Node (support t1 ekimei_1 ekimei_2, ekimei, lst, t2)
          else Node (t1, ekimei, lst, support t2 ekimei_1 ekimei_2) in
        let tree1 = support tree kiten shuten in
        let tree2 = support tree1 shuten kiten in
        tree2

(*Objective: all insert*)
...
(*Objective: faster get_ekikan_kyori*)
(*get_ekikan_kyori : string -> string -> ekikan_Tree_t -> float*)
let get_ekikan_kyori ekimei_1 ekimei_2 tree = 
  (*extract: string -> (string * float) list -> float *)
  (* let rec extract ekimei0 lst = match lst with
    [] -> raise Not_found
  | (ekimei, dist) :: rest ->
      if ekimei = ekimei0 then dist
      else extract ekimei0 rest in *)

  (* let rec search ekimei_1 ekimei_2 tree =
    match tree with
      Empty -> raise Not_found
    | Node (t1, ekimei, lst, t2) ->
        if ekimei = ekimei_1 then extract ekimei_2 lst
        else if ekimei = ekimei_2 then extract ekimei_1 lst
        else if ekimei_1 < ekimei then search ekimei_1 ekimei_2 t1
        else search ekimei_1 ekimei_2 t2 in *)
      
    assoc ekimei_1 (Tree.search ekimei_2 tree)

let ekikan_tree = inserts_ekikan Empty global_ekikan_list
(* let test = get_ekikan_kyori "池袋" "新大塚" global_ekikan_list = get_ekikan_kyori2 "池袋" "新大塚" ekikan_tree
...
let time f =
  ...
  (res, end_ -. start)
let time1 = time (fun () -> get_ekikan_kyori "神田" "三越前" global_ekikan_list)
let time2 = time (fun () -> get_ekikan_kyori2 "神田" "三越前" ekikan_tree) *)